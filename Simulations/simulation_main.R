...
K <- 10    # Number of latent classes: 5 or 10
L <- 10    # Number of blocks: 5 or 10
...
lambda_vec <- c(2, 1, 0.5, 0.1) * sqrt(log(J*K)/N) # use same grid as in CV
opt_lambda_ind <- 2   # this value should be selected by CV
opt_lambda <- lambda_vec[opt_lambda_ind] # tuning parameter in step 3
...
results_matrix <- foreach(n_rep = 1:N_rep, 
  ...
  # ==============================================================================
  # Step 3: Precision Matrix Estimation (Fixed Lambda)
  # ==============================================================================
  t0_step3 <- Sys.time()
  ...
  for (k in 1:K) {
    ...
    for (l_idx in seq_along(L_labels)) {
      ...
      if (min_eig < 1e-4) Sigma_sub <- Sigma_sub + diag(abs(min_eig) + 1e-4, nrow(Sigma_sub))
  ...
  for(k in 1:K) {
    Omega_hat <- Omega_hat_final[[k]]
    Omega_true <- Omega_list[[k]]
    Omega_true[abs(Omega_true) < 0.01] <- 0
    
    acc[k] <- mean((Omega_true == 0) == (Omega_hat == 0))
    acc_F[k] <- norm(Omega_true - Omega_hat, "F")^2 / J^2
  }
  
  rep_acc_Omega <- mean(acc)
  rep_acc_Omega_F <- sqrt(mean(acc_F))
  ...
  c(acc_Z = rep_acc_Z, 
    acc_L = rep_acc_L, 
    acc_V = rep_acc_V, # L_hat = L_hat,
    acc_Omega = 1-rep_acc_Omega, 
    acc_Omega_F = rep_acc_Omega_F, 
...
cat("Hamming dist for Blocks (V_hat):", mean(results_df$acc_V_hamming), "\n") 
cat("Sparsity Error (Omega):    ", mean(results_df$acc_Omega), "\n") 
cat("Frobenius Error (Omega):   ", mean(results_df$acc_Omega_F), "\n")
...
final_summary <- data.frame(
  ...
  acc_L = mean(results_df$acc_L),
  acc_V = mean(results_df$acc_V),
  acc_Omega = mean(results_df$acc_Omega),
  acc_Omega_F = mean(results_df$acc_Omega_F),
  total_time_sec = mean(results_df$time_total)
)
